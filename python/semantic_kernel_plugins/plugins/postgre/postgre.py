...
class PostgrePlugin:
    ...
    def insert_data(self, table_name: str, data: dict) -> str:
        ...
        return self._execute_query(
            f"INSERT INTO {table_name} ({', '.join(data.keys())}) VALUES ({', '.join([f"'{str(v)}'" for v in data.values()])})"
        )
    ...
    def update_data(self, table_name: str, data: dict) -> str:
        ...
        return self._execute_query(
            f"UPDATE {table_name} SET {', '.join([f"{k} = '{str(v)}'" for k, v in data.items()])}"
        )
    ...
    def delete_data(self, table_name: str, data: dict) -> str:
        ...
        return self._execute_query(
            f"DELETE FROM {table_name} WHERE {', '.join([f"{k} = '{str(v)}'" for k, v in data.items()])}"
        )
    ...
    def create_table(self, table_name: str, schema: dict) -> str:
        ...
        return self._execute_query(
            f"CREATE TABLE {table_name} ({', '.join([f"{k} {v}" for k, v in schema.items()])})"
        )
