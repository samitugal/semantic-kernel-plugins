...
class SKLogger:
    ...
    def _format_message(self, level: LogLevel, message: str) -> str:
        ...
        timestamp = f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] " if self.include_timestamp else ""
    ...
    def _log(self, level: LogLevel, message: str, **kwargs):
        ...
        try:
            ...
            formatted_message = self._format_message(level, self._replace_emojis(message))
            self.logger.log(level.level_num, formatted_message)
    ...
    def get_recent_logs(self, count: int = 10, level: Optional[LogLevel] = None) -> List[Tuple[LogLevel, str, float]]:
    ...
    def log_code_generation_process(self, request: str, planning: str, code: str, execution_result: str, success: bool = True):
