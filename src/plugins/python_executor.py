...
class ExecutePythonCodePlugin:
    ...
    def _create_virtual_env(self):
        try:
            ...
            subprocess.run(
                [sys.executable, "-m", "venv", self._venv_dir, "--system-site-packages"],
                check=True,
                capture_output=True,
                timeout=60,
            )
            
            python_cmd = os.path.join(self._venv_dir, "Scripts" if is_windows else "bin", "python")
            pip_cmd = os.path.join(self._venv_dir, "Scripts" if is_windows else "bin", "pip")
            ...
            self._venv_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".venv"))
    ...
    def _execute_in_virtual_env(self, file_path: str, code: str) -> str:
        ...
        try:
            ...
            python_path = os.path.join(self._venv_dir, "Scripts" if is_windows else "bin", "python")
            pip_path = os.path.join(self._venv_dir, "Scripts" if is_windows else "bin", "pip")
            ...
            formatted_result = ""
            if installation_log:
                formatted_result += f"Dependency installation:\n{installation_log[0]}\n\n"
    ...
    def _detect_missing_packages(self, code: str) -> List[str]:
        ...
        try:
            ...
            for match in matches:
                ...
                if root_module in self._restricted_modules and not self._is_exception_allowed(root_module):
                    continue
                ...
                if package_name not in missing_packages and package_name not in ['__future__']:
    ...
    def _install_packages_in_venv(self, packages: List[str], pip_path: str = None) -> str:
        ...
        try:
            ...
            for package in packages:
                if package.lower() in [m.lower() for m in self._restricted_modules]:
                    installation_log.append(f"Cannot install restricted module: {package}")
                    continue
                
                safe_package = re.sub(r'[^a-zA-Z0-9_\.-]', '', package)
                if safe_package != package:
                    installation_log.append(f"Package name contains invalid characters: {package}")
                    continue
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        installation_log.append(f"Failed to install {package}: {result.stderr}")
                        if "No module named pip" in result.stderr:
                            installation_log.append("Pip not found, trying to install pip first...")
                            self._install_pip_manually()
                            python_path = os.path.join(self._venv_dir, "Scripts" if is_windows else "bin", "python")
                            result = subprocess.run(
                                [python_path, "-m", "pip", "install", safe_package],
                            ...
                            if result.returncode == 0:
                                installation_log.append(f"Successfully installed {package} after installing pip")
                            else:
                                installation_log.append(f"Still failed to install {package}: {result.stderr}")
                except Exception as e:
        ...
        try:
            ...
            for match in matches:
                ...
                try:
                    ...
                    if root_module not in missing_packages and root_module not in ['__future__']:
                        missing_packages.append(root_module)
            
            if missing_packages:
                installation_log.append(f"Installing packages: {', '.join(missing_packages)}")
